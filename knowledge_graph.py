...
#         entity_nodes = [n for n in self.graph.nodes() if self.graph.nodes[n].get('type') in ['PERSON', 'ORG', 'GPE', 'MISC']]
#         document_nodes = [n for n in self.graph.nodes() if n not in entity_nodes]
        
#         # Draw nodes
#         nx.draw_networkx_nodes(self.graph, pos, nodelist=entity_nodes, 
#                               node_color='lightblue', node_size=300, alpha=0.7, label='Entities')
#         nx.draw_networkx_nodes(self.graph, pos, nodelist=document_nodes, 
#                               node_color='lightcoral', node_size=500, alpha=0.7, label='Documents')
...
#             node_type = node_info.get('type', 'Document')
#             connections = len(list(self.graph.neighbors(node)))
            
#             node_trace['text'] += (node[:15] + '...' if len(node) > 15 else node,)
#             node_trace['marker']['size'] += (20 + connections * 5,)
#             node_trace['marker']['color'] += (connections,)
...
#             'isolated_node_list': isolated_nodes[:10],  # Show first 10
...
#             if node_data.get('type') in ['PERSON', 'ORG', 'GPE', 'MISC', 'DATE', 'MONEY']:
...
#             connections = len(list(self.graph.neighbors(entity)))
#             if connections < 2:
...
#             'sparse_entities': sparse_entities[:20],  # Top 20 sparse entities
...
#         bridges = list(nx.bridges(self.graph)) if nx.is_connected(self.graph) else []
...
#             'critical_nodes': articulation_points[:10],
...
#         # Build entity-document mappings
#         for edge in self.graph.edges():
#             node1, node2 = edge
#             node1_type = self.graph.nodes[node1].get('type', 'Document')
#             node2_type = self.graph.nodes[node2].get('type', 'Document')
            
#             if node1_type != 'Document' and node2_type == 'Document':
#                 entity_doc_map[node1].add(node2)
#                 doc_entity_map[node2].add(node1)
#             elif node1_type == 'Document' and node2_type != 'Document':
#                 entity_doc_map[node2].add(node1)
#                 doc_entity_map[node1].add(node2)
        
#         # Find entities mentioned in multiple documents
#         cross_document_entities = {e: docs for e, docs in entity_doc_map.items() if len(docs) > 1}
        
#         # Find documents with few entities
#         sparse_documents = {d: entities for d, entities in doc_entity_map.items() if len(entities) < 3}
        
#         return {
#             'cross_document_entities': len(cross_document_entities),
#             'cross_document_examples': list(cross_document_entities.keys())[:10],
#             'sparse_documents': len(sparse_documents),
...
#             recommendations.append(f"Address {structural['isolated_nodes']} isolated nodes by adding more contextual information")
        
#         if structural.get('connectivity_ratio', 0) < 0.7:
#             recommendations.append("Improve graph connectivity by adding more cross-references between documents")
        
#         # Content recommendations
#         if content.get('sparse_entities', []):
#             recommendations.append(f"Expand context for {len(content['sparse_entities'])} sparsely connected entities")
        
#         if content.get('entities_per_document', 0) < 5:
#             recommendations.append("Consider adding more detailed entity extraction or richer documents")
        
#         # Connectivity recommendations
#         if connectivity.get('articulation_points', []):
#             recommendations.append("Strengthen connections around critical nodes to improve graph robustness")
        
#         # Coverage recommendations
#         if coverage.get('cross_document_entities', 0) < 5:
#             recommendations.append("Add more documents that share common entities to improve knowledge linking")
        
#         if not recommendations:
#             recommendations.append("Knowledge graph structure looks healthy!")
...
# - Isolated nodes: {structural.get('isolated_nodes', 0)}
...
# - Total documents: {content.get('total_documents', 0)}
# - Average entities per document: {content.get('entities_per_document', 0):.2f}
# - Sparse entities (< 2 connections): {len(content.get('sparse_entities', []))}
...
# - Critical nodes (articulation points): {connectivity.get('articulation_points', 0)}
# - Degree distribution: {connectivity.get('degree_distribution', {})}
# """

#     def _format_coverage_analysis(self, coverage):
#         return f"""
# - Cross-document entities: {coverage.get('cross_document_entities', 0)}
# - Sparse documents (< 3 entities): {coverage.get('sparse_documents', 0)}
# - Average entities per document: {coverage.get('average_entities_per_document', 0):.2f}
# """
...
#     # Assuming you have a rag_system instance
#     # results = analyze_rag_knowledge_graph(rag_system)
#     pass
...
class KnowledgeGraphVisualizer:
    ...
    def visualize_graph_interactive(self, height=800):
        ...
        for node_id in self.graph.nodes():
            ...
            if incoming_edges or outgoing_edges:
                ...
                for u, v, data in incoming_edges:
                    hover_content += f"<br> &larr; {data.get('relation_type', 'related to')} {u}"
                for u, v, data in outgoing_edges:
                    hover_content += f"<br> &rarr; {data.get('relation_type', 'related to')} {v}"
        ...
        for edge in self.graph.edges(data=True):
            ...
            arrow_x_offset = (x1 - x0) * 0.05
            arrow_y_offset = (y1 - y0) * 0.05
